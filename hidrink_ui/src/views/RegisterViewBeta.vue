      ...
      >
        ...
        <div style="margin-top: 50px; margin-right: 100px; float: right">
          <el-form
            ...
            style="float: right"
          >
            <el-form-item label="注册角色：" :required="true">
              <el-radio  v-model="ruleForm.role" :label="1" @click="Choose_customE()">员工</el-radio>
              <el-radio  v-model="ruleForm.role" :label="2" @click="Choose_customC()">顾客</el-radio>
            </el-form-item>
            ...
            <el-form-item label="就职部门" prop="deptId" :required="true" id="emp" v-show="ruleForm.role == 1">
...
export default {
  ...
  methods: {
    ...
    getDeptItem() {
      var values;
      var outside = this;
      let deptUrl = outside.GLOBAL.networkSrc+"/deptList";
      var config = {
        headers: {
          "Content-Type": "application/json",
          "Access-Control-Allow-Origin": "*",
        ...
        .then(function (result) {
          ...
          if (serverResponse.code === 1) {
            ...
            values.forEach((item) => {
              var sub = '{"value":' + item + "," + "";
              sub += '"label":"' + data[item] + '"}' + "";
              outside.deptItem.push(JSON.parse(sub));
            });
            console.log(outside.deptItem);
          } else {
            outside.$message({
              type: "error",
              message: serverResponse.message,
